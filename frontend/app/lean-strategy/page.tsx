    ...
    <div className="space-y-6 pb-16">
      ...
      <section className="rounded-2xl border border-slate-800 bg-slate-900/70 px-5 py-4">
        ...
        <h1 className="mt-1 text-2xl font-semibold text-slate-100">
          {lang === "zh" ? "ABC / XYZ 分类分析" : "ABC / XYZ Classification"}
        </h1>
        <p className="mt-1 text-sm text-slate-400">
          {lang === "zh"
            ? "ABC 按销售额贡献分层，XYZ 按需求稳定性分层，组合指导库存策略。"
            : "ABC by sales contribution, XYZ by demand variability — combined to guide inventory strategy."}
        </p>
        {data && (
